...
impl From<ConnectionConfig> for WebSocketConfig {
	fn from(config:ConnectionConfig) -> Self {
		// Disabling the warning on max_send_queue which we don't use anymore
	...
	if let Some(write) = manager.0.lock().await.get_mut(&id) {
		write
			.send(match message {
				...
				WebSocketMessage::Close(t) => {
					Message::Close(t.map(|v| {
						ProtocolCloseFrame {
							code:v.code.into(),
...
pub fn init<R:Runtime>() -> TauriPlugin<R> { Builder::default().build() }
...
impl Builder {
	pub fn new() -> Self { Self { tls_connector:None } }
